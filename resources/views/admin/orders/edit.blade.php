  ...
  <div class="flex min-h-[calc(100vh-100px)] flex-col">
    ...
    <div class="mb-6 flex shrink-0 items-center justify-between">
      <div>
        ...
        <p class="mt-0.5 text-sm text-gray-500">Order #{{ $order->id }} • {{ $order->created_at->format('M d, Y') }}</p>
      </div>
  ...
  <script>
    function orderForm() {
      return {
        ...
        selectedUserEmail: '{{ $order->user?->email ?? '' }}',
        customerSearch: '{{ $order->user?->name ?? ($order->shipping_address['name'] ?? '') }}',
        customerDropdownOpen: false,
        customerResults: [],
        isLoadingCustomers: false,

        userAddresses: @json($order->user?->addresses ?? []),
        addressDropdownOpen: false,

        items: @json($orderItemsForJs),
        shipping: @json($shippingForJs),
