...
@section('content')
    <div class="space-y-6">
        ...
        <form action="{{ route('admin.orders.update', $order) }}" method="POST" x-data="orderForm()" @click.outside="closeDropdowns()">
            @csrf
            @method('PUT')
                                ...
                                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                                    ...
                                    <tr x-show="items.length === 0">
                                        <td colspan="5" class="px-6 py-8 text-center text-gray-500 dark:text-gray-400">
                                            Search and select a product above to add items.
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
                     ...
                     <x-admin.ui.card>
                         <div class="p-6">
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Order Notes</label>
                             <textarea name="notes" rows="3" class="w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-800 shadow-sm focus:border-primary focus:ring focus:ring-primary focus:ring-opacity-50">{{ old('notes', $order->notes) }}</textarea>
                         </div>
                    </x-admin.ui.card>
                        ...
                        <div class="border-b border-gray-200 dark:border-gray-700 px-6 py-4 bg-gray-50 dark:bg-gray-800/50">
                         ...
                         <div class="p-6 space-y-3">
                            ...
                            <div class="flex flex-col gap-3 mt-6">
                                <button type="submit" class="w-full py-2.5 px-4 border border-transparent rounded-lg shadow-sm text-sm font-medium text-white bg-primary hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary transition-all">
                                    Update Order
                                </button>
                                <a href="{{ route('admin.orders.invoice.create', $order) }}" class="w-full text-center py-2.5 px-4 border border-gray-300 dark:border-gray-600 rounded-lg shadow-sm text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700 transition-all">
                                    Generate Invoice
                                </a>
                            </div>
                        </div>
    ...
    <script>
        function orderForm() {
            return {
                ...
                customerSearch: '{{ addslashes($order->user ? $order->user->name : ($order->shipping_address['name'] ?? '')) }}',
                ...
                items: [
                    @foreach($order->items as $item)
                    {
                        ...
                        code: '{{ addslashes($item->product->product_code ?? '') }}',
                ...
                shipping: {
                    ...
                    email: '{{ addslashes($order->shipping_address['email'] ?? '') }}',
                    phone: '{{ addslashes($order->shipping_address['phone'] ?? '') }}',
                    address: '{{ addslashes($order->shipping_address['address'] ?? '') }}',
                    city: '{{ addslashes($order->shipping_address['city'] ?? '') }}',
                    postal_code: '{{ addslashes($order->shipping_address['postal_code'] ?? '') }}',
                    country: '{{ addslashes($order->shipping_address['country'] ?? '') }}'
                },
                ...
                selectCustomer(user) {
                    ...
                    // Auto-fill shipping details (optional in edit, maybe ask before overwriting?)
