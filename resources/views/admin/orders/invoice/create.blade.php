            ...
            return {
                ...
                items: {
                    @foreach($items as $itemData)
                        '{{ $itemData['item']->id }}': {
                            @php
                                $pcs = (float)($itemData['item']->product?->pcs_in_ctn ?: 1);
                                $qty = (float)$itemData['item']->quantity;
                                $isBox = ($pcs > 0 && fmod($qty, $pcs) == 0);
                            @endphp
                            selected: true,
                            calc_type: '{{ $isBox ? "box" : "quantity" }}',
                            boxes: {{ $isBox ? ($qty / $pcs) : 1 }},
                            qty_per_box: {{ $pcs }},
                            quantity: {{ $qty }},
