    ...
    <div class="space-y-6">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-sm text-gray-500 dark:text-gray-400">Invoice #{{ $invoice->invoice_number }} • Order #{{ $invoice->order_id }}</p>
            </div>
        ...
        <form action="{{ route('admin.invoices.update', $invoice) }}" method="POST" x-data="invoiceItems()">
            @csrf
            @method('PUT')
                                            ...
                                            <td class="px-4 py-4 align-top">
                                                ...
                                                <template x-if="items['{{ $itemId }}'].selected">
                                                    <div>
                                                        ...
                                                        <input type="hidden" name="items[{{ $itemId }}][price]" :value="items['{{ $itemId }}'].calc_type === 'box' ? (items['{{ $itemId }}'].qty_per_box > 0 ? (items['{{ $itemId }}'].box_price / items['{{ $itemId }}'].qty_per_box).toFixed(4) : 0) : items['{{ $itemId }}'].price">
                                            ...
                                            <td class="px-4 py-4 align-top text-right" x-data="{ editing: false }">
                                                <div class="relative group" @click.away="editing = false">
                                                    ...
                                                    <div x-show="!editing" class="flex items-center justify-end gap-2 cursor-pointer py-1" @click="if(items['{{ $itemId }}'].selected) editing = true">
                                                        <span class="font-medium text-gray-900 dark:text-white" x-text="formatMoney(items['{{ $itemId }}'].calc_type === 'box' ? items['{{ $itemId }}'].box_price : items['{{ $itemId }}'].price)"></span>
                                                        <svg class="w-3 h-3 text-gray-400 group-hover:text-primary transition-colors opacity-0 group-hover:opacity-100" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg>
                                                    </div>
                                                    <!-- Edit Mode -->
                    ...
                    <!-- Notes Section -->
                     <x-admin.ui.card>
                        <div class="p-6">
                             ...
                             <textarea name="notes" rows="3" class="w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-800 dark:text-white shadow-sm focus:border-primary focus:ring focus:ring-primary focus:ring-opacity-50 placeholder-gray-400">{{ old('notes', $invoice->notes) }}</textarea>
                        </div>
                    </x-admin.ui.card>
                </div>
                        ...
                        <div class="p-6 space-y-4">
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="date" name="invoice_date" value="{{ old('invoice_date', $invoice->invoice_date->format('Y-m-d')) }}" class="w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-800 dark:text-white shadow-sm focus:border-primary focus:ring focus:ring-primary focus:ring-opacity-50 text-sm">
                                    @error('invoice_date') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Due Date</label>
                                    <input type="date" name="due_date" value="{{ old('due_date', $invoice->due_date->format('Y-m-d')) }}" class="w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-800 dark:text-white shadow-sm focus:border-primary focus:ring focus:ring-primary focus:ring-opacity-50 text-sm">
                        ...
                        <div class="p-6 space-y-3">
                            ...
                            <div class="mt-6 flex flex-col gap-3">
                                <button type="submit" class="w-full flex justify-center items-center py-2.5 px-4 border border-transparent rounded-lg shadow-sm text-sm font-medium text-white bg-primary hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary transition-all">
                                 ...
                                 <a href="{{ route('admin.invoices.print', $invoice) }}" target="_blank" class="w-full flex justify-center items-center py-2.5 px-4 border border-gray-300 dark:border-gray-600 rounded-lg shadow-sm text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary transition-all">
        ...
        function invoiceItems() {
            return {
                ...
                items: {
                    @foreach($invoice->items as $item)
                        '{{ $item->id }}': {
                            ...
                            calc_type: 'quantity',
                            boxes: 1,
                            qty_per_box: parseFloat('{{ $item->product?->pcs_in_ctn ?: 1 }}'),
                            quantity: {{ $item->quantity }},
                            price: {{ $item->unit_price }},
                            box_price: parseFloat('{{ $item->product?->box_price ?? ($item->unit_price * ($item->product?->pcs_in_ctn ?: 1)) }}'),
                            discountValue: {{ $item->discount_amount }},
                            discountType: 'fixed',
                            tax_rate: {{ $item->product && $item->product->tax ? $item->product->tax->value : 0 }}
                        },
                    @endforeach
                },
                toggleAll(e) {
                ...
                calculateItemDiscount(item) {
                     let subtotal = item.calc_type === 'box' ? (item.boxes * item.box_price) : (item.quantity * item.price);
                ...
                calculateLineTotal(item) {
                     ...
                     const subtotal = item.calc_type === 'box' ? (item.boxes * item.box_price) : (item.quantity * item.price);
