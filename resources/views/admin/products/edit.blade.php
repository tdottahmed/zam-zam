                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                        ...
                        <div>
                            ...
                            <div class="mt-1 flex gap-2">
                                <x-admin.form.input id="quantity" name="quantity" type="number" min="0" :value="old('quantity', $product->quantity)" class="flex-1 min-w-0" />
                                <select id="stock_unit" name="stock_unit" class="block w-28 rounded-md border-gray-300 shadow-sm focus:border-[#C41E3A] focus:ring-[#C41E3A] sm:text-sm">
                                    @foreach(\App\Models\Product::stockUnitOptions() as $value => $label)
                                        <option value="{{ $value }}" {{ old('stock_unit', $product->stock_unit ?? 'piece') == $value ? 'selected' : '' }}>{{ $label }}</option>
                                    @endforeach
                                </select>
                            </div>
