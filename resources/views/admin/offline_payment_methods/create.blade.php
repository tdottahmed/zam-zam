...
@section('content')
    ...
    <form method="POST" action="{{ route('admin.offline-payment-methods.store') }}" x-data="paymentMethodForm()">
        @csrf
                ...
                <x-admin.ui.card>
                    <div class="p-6 space-y-6">
                        ...
                        <div>
                            ...
                            <x-admin.form.input id="name" name="name" :value="old('name')" placeholder="e.g. Bank Transfer" required autofocus />
                            <x-admin.form.input-error :messages="$errors->get('name')" class="mt-2" />
                            <p class="text-xs text-gray-500 mt-1">This will be displayed to the customer during checkout.</p>
                        </div>

                        <!-- Description -->
                        <div>
                            <x-admin.form.label for="description" value="Description / Instructions" />
                            <textarea id="description" name="description" rows="3" class="w-full rounded-lg border-gray-300 focus:border-[#C41E3A] focus:ring focus:ring-[#C41E3A] focus:ring-opacity-20 transition shadow-sm text-sm" placeholder="Provide instructions to the customer (e.g. Please transfer to Account No: 123456789)">{{ old('description') }}</textarea>
                                        ...
                                        <div class="flex-1 grid grid-cols-1 md:grid-cols-2 gap-4">
                                            ...
                                            <div class="flex items-center mt-6">
                                                <label class="inline-flex items-center cursor-pointer">
                                                    <input type="checkbox" x-model="field.is_required" :name="'required_fields['+index+'][is_required]'" value="1" class="rounded border-gray-300 text-[#C41E3A] shadow-sm focus:ring-[#C41E3A]">
                ...
                <x-admin.ui.card>
                    <div class="p-6">
                        ...
                        <label for="is_active" class="inline-flex items-center cursor-pointer mb-6">
                            <input id="is_active" type="checkbox" class="rounded border-gray-300 text-[#C41E3A] shadow-sm focus:ring-[#C41E3A]" name="is_active" value="1" {{ old('is_active', true) ? 'checked' : '' }}>
    ...
    <script>
        function paymentMethodForm() {
            return {
                fields: @json(old('required_fields', [])),
                addField() {
